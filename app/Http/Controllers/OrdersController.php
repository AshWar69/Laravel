...
class OrdersController extends Controller
{
    public function index(){
        ...
        return view('checkout.review_order',compact('shipping_address','cart_datas','total_price','cart_count'));
    }
    ...
    public function cod(){
        ...
        return view('payment.cod',compact('user_order','cart_count'));
    }
    public function paypal(Request $request){
        $session_id=Session::get('session_id');
        $cart_count=Cart_model::where('session_id',$session_id)->count();
        $who_buying=Orders_model::where('users_id',Auth::id())->first();
        return view('payment.paypal',compact('who_buying','cart_count'));
    }
}
